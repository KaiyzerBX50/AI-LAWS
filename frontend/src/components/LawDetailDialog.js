...
import { MapPin, ExternalLink, Building2, Calendar, Layers, Link2, ShieldCheck, Landmark, AlertCircle } from 'lucide-react';
          ...
          <ScrollArea className="max-h-[88vh]">
            <div className="p-6">
              <DialogHeader className="space-y-3 text-left">
                ...
                {law.status_raw && (
                  <p className="text-xs text-muted-foreground">{law.status_raw}</p>
                )}
              ...
              {law.sources?.[0]?.url && (
                <a
                  href={law.sources[0].url}
                  target="_blank"
                  rel="noopener noreferrer"
                  data-testid="law-primary-source-button"
                  className="mt-4 flex items-center justify-center gap-2 rounded-lg bg-gradient-to-br from-primary to-accent px-4 py-2.5 text-sm font-semibold text-primary-foreground shadow-lg transition-transform duration-200 hover:-translate-y-0.5"
                >
              ...
              {law.notes && /verify/i.test(law.notes) && (
                <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-400/40 bg-amber-500/10 px-3 py-2 text-xs leading-relaxed text-amber-700 dark:text-amber-300">
              ...
              <div className="mt-4 grid grid-cols-1 gap-2 rounded-lg border border-border bg-secondary/50 p-3 text-sm sm:grid-cols-2">
                ...
                <Meta icon={Landmark} label="Level" value={law.level} />
